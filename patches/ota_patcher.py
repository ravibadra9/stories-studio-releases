...
def purge_stale_patches(log_fn: Optional[Callable[[str], None]] = None) -> bool:
    """
    Ensure no stale hot-patches from previous builds, older projects, or legacy releases
    ever override the fresh base application (e.g. fresh 1.0.0 start).
    """
    try:
        from updater import CURRENT_VERSION, parse_version
        info = _load_active_patch_info()
        patch_ver = info.get("version", "0.0.0")
        base_app_ver = info.get("base_app_version", "")
        installed = info.get("installed_patches", [])

        # If base_app_version doesn't match CURRENT_VERSION,
        # OR if patch_ver was from a legacy cycle,
        # OR if base_version >= patch_version:
        # PURGE IMMEDIATELY so the fresh code runs cleanly!
        is_legacy = (base_app_ver != CURRENT_VERSION)
        if is_legacy or (patch_ver != "0.0.0" and (installed or os.path.exists(HOT_PATCH_DIR))):
            base_v = parse_version(CURRENT_VERSION)
            pat_v = parse_version(patch_ver)
            if is_legacy or (base_v >= pat_v):
                _safe_log(log_fn, f"[OTA] Purging legacy/stale hot patches (app v{CURRENT_VERSION}, patch v{patch_ver}).")
                if os.path.exists(HOT_PATCH_DIR):
                    shutil.rmtree(HOT_PATCH_DIR, ignore_errors=True)
                os.makedirs(HOT_PATCH_DIR, exist_ok=True)
                new_status = {
                    "base_app_version": CURRENT_VERSION,
        ...
        _safe_log(log_fn, f"[OTA] Notice during patch validation: {e}")
    return False
...
def setup_hot_patch_path():
    ...
    try:
        ...
        # Only mount finder and inject modules if there are actual active patches
        if info.get("installed_patches"):
...
def check_and_apply_cloud_patches(
    ...
    try:
        ...
        manifest = None
        # 1. Primary: Firebase Realtime Cloud (instant 0-second cache propagation)
        try:
        ...
        # 2. Secondary: GitHub Raw Manifest fallback
        if not manifest:
        ...
        if not manifest:
            result["message"] = "Could not fetch manifest from cloud sources."
            return result
        ...
        # Do not apply patches if base app version is already >= patch_ver
        try:
            from updater import CURRENT_VERSION, parse_version
            if parse_version(CURRENT_VERSION) >= parse_version(patch_ver):
                result["success"] = True
                result["message"] = f"Base app (v{CURRENT_VERSION}) already incorporates all patches up to v{patch_ver}."
                if current_info.get("installed_patches") or os.path.exists(HOT_PATCH_DIR):
                    purge_stale_patches(log_fn)
                return result
        except Exception:
            pass
        ...
        if not files_to_patch:
            ...
            # Clean up stale files in HOT_PATCH_DIR if manifest was reset to empty
            if current_info.get("installed_patches") or current_ver != patch_ver:
                try:
                    for item in os.listdir(HOT_PATCH_DIR):
                        if item != "patch_status.json":
                            item_p = os.path.join(HOT_PATCH_DIR, item)
                    ...
                    with open(PATCH_INFO_FILE, "w", encoding="utf-8") as f:
                        json.dump({"version": patch_ver, "installed_patches": []}, f)
                except Exception:
                    pass
            return result
