...
_FEMALE_VOICE_NAMES = {
    "bella", "rachel", "domi", "sarah", "elli", "freya", "gigi", "glinda",
    "grace", "charlotte", "emily", "matilda", "mimi", "nicole", "swara",
    "neerja", "jenny", "aria", "sonia", "female", "woman", "girl"
}


def resolve_fallback_edge_voice(voice_id: str, text: str) -> str:
    """Map any voice ID or character name to the best matching Edge Neural voice."""
    raw = str(voice_id).lower()
    if "edge_" in raw or "neural" in raw:
        v = str(voice_id).replace("edge_", "").strip()
        return v

    is_hindi = any('\u0900' <= char <= '\u097f' for char in text)
    is_female = any(fem in raw for fem in _FEMALE_VOICE_NAMES)
...
def generate_spoken_fallback_tts(text: str, voice_id: str = "", out_path: str = "", speed: float = 1.0, log_fn = None) -> bool:
    """
    High-fidelity Zero-Failure Free Fallback TTS:
    1. Microsoft Edge Neural TTS (100% free, natural neural speech, multi-lingual)
    2. Google Translate gTTS (secondary zero-dependency safety net)
    """
    ...
    try:
        ...
        edge_voice = resolve_fallback_edge_voice(voice_id, text)
        speed_pct = int(round((float(speed) - 1.0) * 100))
        rate_str = f"+{speed_pct}%" if speed_pct >= 0 else f"{speed_pct}%"

        if log_fn:
            log_fn(f"[fallback-edge-tts] Synthesizing speech with '{edge_voice}' (rate {rate_str})...")

        async def _edge_runner():
            comm = edge_tts.Communicate(text=text.strip(), voice=edge_voice, rate=rate_str)
            await comm.save(out_path)

        loop = asyncio.new_event_loop()
        try:
            asyncio.set_event_loop(loop)
            loop.run_until_complete(_edge_runner())
        finally:
            try:
                loop.close()
            except Exception:
        ...
        if os.path.exists(out_path) and os.path.getsize(out_path) > 200:
            if log_fn:
                log_fn(f"[fallback-edge-tts] Successfully generated audio ({os.path.getsize(out_path)} bytes)")
            return True
    except Exception as exc:
        if log_fn:
            log_fn(f"[fallback-edge-tts] Edge TTS error: {exc}. Trying gTTS...")
    ...
    try:
        ...
        if log_fn:
            log_fn(f"[fallback-gtts] Synthesizing speech with Google TTS (lang={lang})...")
        ...
        if os.path.exists(out_path) and os.path.getsize(out_path) > 200:
            if log_fn:
                log_fn(f"[fallback-gtts] Successfully generated audio ({os.path.getsize(out_path)} bytes)")
            return True
    except Exception as exc:
        if log_fn:
            log_fn(f"[fallback-gtts] Google TTS error: {exc}")
...
def ai33_tts_generate(
    ...
    if log_fn:
        log_fn(f"[ai33-tts] Primary API unavailable, activating free neural fallback TTS (Edge-TTS / gTTS)...")
    return generate_spoken_fallback_tts(text=text, voice_id=prefixed_vid or clean_vid, out_path=out_path, speed=speed, log_fn=log_fn)
