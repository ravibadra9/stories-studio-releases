...
def add_watermark(video_path: str, logo_path: str, out_path: str, render_preset: str = "fast",
                  ...
                  preview_base_w: int = 1280, preview_base_h: int = 720):
    """Add watermark. Offsets/size/custom coords are scaled from the 1280x720 preview
    space to the actual output resolution so the logo lands exactly where the user placed it."""
    pos = position.strip()

    # Scale factors: preview canvas was 1280x720. Real output may be 1920x1080, 1920x960, etc.
    prof = _get_full_profile(video_path)
    out_w = prof.get("width", 1280)
    out_h = prof.get("height", 720)
    sf_x = out_w / float(max(1, preview_base_w))
    sf_y = out_h / float(max(1, preview_base_h))
    sf_w = sf_x  # scale logo width proportionally to video width

    scaled_logo_w = max(20, min(out_w, int(round(logo_width * sf_w))))
    sx = int(round(x_offset * sf_x))
    sy = int(round(y_offset * sf_y))
    scale_expr = f"scale={scaled_logo_w}:-1"

    if custom_x is not None and custom_y is not None and ("custom" in pos.lower() or pos == "Custom"):
        final_cx = max(0, min(out_w - 10, int(round(custom_x * sf_x))))
        final_cy = max(0, min(out_h - 10, int(round(custom_y * sf_y))))
        overlay_expr = f"{final_cx}:{final_cy}"
    elif pos == "Top-Left":
        ...
        final_cx = max(0, min(out_w - 10, int(round(cx * sf_x))))
        final_cy = max(0, min(out_h - 10, int(round(cy * sf_y))))
        overlay_expr = f"{final_cx}:{final_cy}"
...
def build_movie(recap_video_path: str, intro_video_paths: List[str], target_duration_seconds: float,
    ...
    if target_duration_seconds > 0:
        ...
        _log(log_callback, f"[movie] Assembled raw stream ({raw_dur:.1f}s / {raw_dur/3600:.2f}h). Trimming to exact target {target_duration_seconds:.1f}s ({target_duration_seconds/3600:.2f}h)...")
        
        trimmed_temp = os.path.join(workdir, "movie_trimmed_temp.mp4")
        try:
            # Fast stream copy trim with timestamp normalization
            run([
                "ffmpeg", "-y", "-ss", "0", "-i", raw_concat,
                "-t", f"{target_duration_seconds:.3f}",
                "-map", "0:v:0", "-map", "0:a:0?",
                "-c:v", "copy", "-c:a", "aac", "-b:a", "192k",
                "-avoid_negative_ts", "make_zero",
                "-movflags", "+faststart", trimmed_temp
            ])
            check_dur = get_duration(trimmed_temp)
        except Exception:
            check_dur = 0.0

        # If copy resulted in deviation from target or failed, do a precision re-encode trim
        if abs(check_dur - target_duration_seconds) > 1.0 or not _file_nonempty(trimmed_temp):
            _log(log_callback, f"[movie] Precision frame re-encode trim for exact {target_duration_seconds:.1f}s limit (copy was {check_dur:.1f}s)...")
            run([
                "ffmpeg", "-y", "-ss", "0", "-i", raw_concat,
                "-t", f"{target_duration_seconds:.3f}",
                "-map", "0:v:0", "-map", "0:a:0?",
                ...
                "-c:a", "aac", "-b:a", "192k",
                "-avoid_negative_ts", "make_zero",
                "-movflags", "+faststart", out_movie_path
            ])
        else:
            if os.path.exists(out_movie_path):
                try: os.remove(out_movie_path)
                except OSError: pass
            shutil.move(trimmed_temp, out_movie_path)
    else:
    ...
    final_dur = get_duration(out_movie_path)
    _log(log_callback, f"[movie] Final movie created: {out_movie_path} ({final_dur:.1f}s / {final_dur/3600:.2f}h)")
...
def build_project(video_path: str, script_path: str, edit_plan_path: str,
    ...
    logo_meta_file = os.path.join(workdir, "logo_cache_meta.json")
    
    # 1. Logo Cache Invalidation Check
    cur_logo_fp = _sha256_text(
        f"{logo_path}_{enable_logo}_{logo_position}_{logo_width}_{custom_logo_x}_{custom_logo_y}_{logo_x_offset}_{logo_y_offset}"
    )
    prev_logo_fp = ""
    if os.path.exists(logo_meta_file):
        try:
            with open(logo_meta_file, "r", encoding="utf-8") as lmf:
                prev_logo_fp = json.load(lmf).get("fingerprint", "")
        except Exception:
            pass

    logo_changed = (prev_logo_fp != cur_logo_fp)
    if logo_changed:
        for stale_f in [watermark_out, os.path.join(workdir, "with_captions.mp4"), os.path.join(workdir, "recap_base.mp4"), out_abs]:
    ...
    if enable_logo and logo_path and os.path.exists(logo_path):
        ...
        if not logo_changed and _output_is_fresh(watermark_out, wm_inputs):
            _log(log_callback, "[cache] Watermarked video reused (100% complete)")
        else:
            pos_desc = f"pos=({custom_logo_x}, {custom_logo_y})" if ("custom" in logo_position.lower() and custom_logo_x is not None) else logo_position
            _log(log_callback, f"[final] [0%] Adding logo watermark ({pos_desc}, width={logo_width}px)...")
            add_watermark(current, logo_path, watermark_out, render_preset,
                          ...
                          log_callback=log_callback)
            try:
                with open(logo_meta_file, "w", encoding="utf-8") as lmf:
                    json.dump({"fingerprint": cur_logo_fp}, lmf)
            except Exception:
                pass
    ...
    # 2. CapCut Subtitles / Captions Burning & Cache Invalidation
    caption_out = os.path.join(workdir, "with_captions.mp4")
    caption_meta_file = os.path.join(workdir, "caption_cache_meta.json")
    cur_caption_fp = _sha256_text(
        f"{enable_captions}_{caption_font}_{caption_size}_{caption_preset}_{caption_position}_{caption_case}_{caption_words_per_line}_{custom_font_path}"
    )
    prev_caption_fp = ""
    if os.path.exists(caption_meta_file):
        try:
            with open(caption_meta_file, "r", encoding="utf-8") as cmf:
                prev_caption_fp = json.load(cmf).get("fingerprint", "")
        except Exception:
            pass

    caption_changed = (prev_caption_fp != cur_caption_fp) or logo_changed
    if caption_changed:
        for stale_f in [caption_out, os.path.join(workdir, "recap_base.mp4"), out_abs]:
            if os.path.exists(stale_f):
                try: os.remove(stale_f)
                except Exception: pass
    ...
    if enable_captions:
        ...
        generate_capcut_ass_file(beats, seg_durations, ass_path,
                                ...
                                text_case=caption_case, max_words_per_line=caption_words_per_line,
                                custom_font_path=custom_font_path)
        if not caption_changed and _output_is_fresh(caption_out, [current, ass_path]):
            ...
            try:
                with open(caption_meta_file, "w", encoding="utf-8") as cmf:
                    json.dump({"fingerprint": cur_caption_fp}, cmf)
            except Exception:
                pass
...
if __name__ == "__main__":
    ...
    p.add_argument("--custom-logo-x", type=int, default=None)
    p.add_argument("--custom-logo-y", type=int, default=None)
    ...
    build_project(
        ...
        custom_logo_x=a.custom_logo_x,
        custom_logo_y=a.custom_logo_y,
    )
