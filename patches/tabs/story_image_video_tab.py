...
class StoryImageVideoTab(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        # 1. Cinematic Camera Animation & Motion
        ctk.CTkLabel(c6, text="Camera Motion").grid(row=1, column=0, padx=6, pady=3, sticky="w")
        self.camera_motion = ctk.CTkComboBox(
            c6, height=26,
            values=[
                "🌟 Auto-Director Flow (Smart Pan & Slow Zoom)",
                "🎬 Slow Push-In (Cinematic Depth Zoom)",
                "🌊 3D Parallax & Gentle Float",
                "↔️ Smooth Cinematic Pan (Left to Right)",
                "🌬️ Living Breathing Camera (Organic Drift)",
                "📐 Subtle Diagonal Drift (Corner Pan)",
                "📽️ Gentle Crane & Tilt Up",
                "⏹️ Static (No Motion / Crisp Image)"
            ],
            state="readonly", fg_color=C_BG, border_color=C_BORDER
        )
        self.camera_motion.grid(row=1, column=1, columnspan=2, padx=(2, 6), pady=3, sticky="ew")
        self.camera_motion.set("🌟 Auto-Director Flow (Smart Pan & Slow Zoom)")
        # Legacy alias for backward compatibility
        self.motion_effect = self.camera_motion

        # 2. Atmospheric Dust & Film Grain FX
        ctk.CTkLabel(c6, text="Atmosphere & FX").grid(row=2, column=0, padx=6, pady=3, sticky="w")
        self.atmosphere_fx = ctk.CTkComboBox(
            c6, height=26,
            values=[
                "✨ Cinematic Dust Particles",
                "🎞️ 35mm Film Grain (Vintage Texture)",
                "📽️ Film Grain + Dust Particles Combo",
                "🌟 Shimmer & Sparkle Glow",
                "🔥 Warm Cinematic Embers",
                "🌧️ Ambient Rain Motes",
                "❄️ Gentle Winter Snow",
                "⏹️ None (Clean / Crisp)"
            ],
            state="readonly", fg_color=C_BG, border_color=C_BORDER
        )
        self.atmosphere_fx.grid(row=2, column=1, columnspan=2, padx=(2, 6), pady=3, sticky="ew")
        self.atmosphere_fx.set("✨ Cinematic Dust Particles")
        ...
        ctk.CTkLabel(c6, text="Visualizer Opacity").grid(row=5, column=0, padx=6, pady=2, sticky="w")
        ...
        self.vis_opacity_slider.grid(row=5, column=1, padx=4, pady=2, sticky="ew")
        ...
        self.vis_opacity_label.grid(row=5, column=2, padx=6)

        # 4. Subscribe Button Overlay (Bottom-Left Corner periodic loop - Auto Background Removed)
        self.enable_subscribe = ctk.CTkCheckBox(c6, text="Subscribe Button (Auto-BG Removed)", text_color=C_TEXT)
        self.enable_subscribe.grid(row=6, column=0, columnspan=3, padx=6, pady=(5, 2), sticky="w")
        self.enable_subscribe.select()

        self.subscribe_entry = self._file_row(c6, 7, "Subscribe GIF / File", self._choose_subscribe, "Subscribe animation GIF (Downloads/subscribe...)")
        ...
        sub_ctrl_wrap.grid(row=8, column=0, columnspan=3, padx=6, pady=2, sticky="ew")
        ...
        # 5. Background Videos & Intros
        self.bg_video_entry = self._file_row(c6, 9, "Background Videos", self._choose_bg_videos, "Optional video backgrounds (.mp4)")

        ctk.CTkLabel(c6, text="BG Video Opacity").grid(row=10, column=0, padx=6, pady=3, sticky="w")
        ...
        self.bg_opacity_slider.grid(row=10, column=1, padx=4, pady=3, sticky="ew")
        ...
        self.bg_opacity_label.grid(row=10, column=2, padx=6)

        self.intro_entry = self._file_row(c6, 11, "Intro / Outro Videos", self._choose_intros, "Optional intro/outro videos (.mp4)")
    ...
    def _apply_preset_longform(self):
        ...
        self.caption_size.set(28)
        self.camera_motion.set("🎬 Slow Push-In (Cinematic Depth Zoom)")
        ...
        def worker():
            try:
                ...
                out = build_story_image_project(
                    ...
                    custom_font_path=font_p if font_p and os.path.exists(font_p) else None,
                    custom_caption_y=self.custom_caption_y,
                    motion_effect=self.camera_motion.get() if hasattr(self, "camera_motion") else "🌟 Auto-Director Flow (Smart Pan & Slow Zoom)",
                    camera_motion=self.camera_motion.get() if hasattr(self, "camera_motion") else "🌟 Auto-Director Flow (Smart Pan & Slow Zoom)",
                    atmosphere_fx=self.atmosphere_fx.get() if hasattr(self, "atmosphere_fx") else "✨ Cinematic Dust Particles",
    ...
    def _apply_settings(self, st: dict):
        ...
        try:
            ...
            if "atmosphere_fx" in st and hasattr(self, "atmosphere_fx"):
                try: self.atmosphere_fx.set(st["atmosphere_fx"])
                except Exception: pass
